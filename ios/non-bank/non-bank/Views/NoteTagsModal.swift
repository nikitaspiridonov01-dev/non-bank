...
struct NoteTagsModal: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                // Warm-cream page surface. Without this, the modal falls
                // back to `systemBackground` (pure white in light) and
                // clashes with the warm-cream atmosphere of the calling
                // screen — title field looked teleported in from a
                // different app. The TextEditor below already has
                // `scrollContentBackground(.hidden)` so the cream shows
                // through there too.
                ...
                VStack(spacing: 0) {
                    ...
                    // `prompt:` lets us paint the placeholder with
                    // `textDisabled` (warm `#B8A695`) so the empty
                    // title matches the "Write a note…" placeholder
                    // below instead of falling back to the system
                    // blue-grey placeholder color.
                    TextField("", text: $title, prompt: Text(placeholderTitle).foregroundColor(AppColors.textDisabled))
                        ...
                        .onChange(of: title) { newValue in
                            // Strip any newline that slipped in (paste,
                            // dictation, hardware keyboard) — the keyboard's
                            // return key now submits via `onSubmit`, but
                            // other input paths can still inject `\n`.
